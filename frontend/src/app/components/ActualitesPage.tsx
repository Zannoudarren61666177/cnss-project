                      ...
                      <div className="p-8 md:p-10 flex flex-col justify-center">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-400 mb-6">
                          <span className="flex items-center gap-1">
                            ...
                            {formatDate(featured.date_publication ?? featured.date)}
                          </span>
                          {featured.temps_lecture && (
                            <span className="flex items-center gap-1">
                              <Clock className="w-3.5 h-3.5" />{featured.temps_lecture} de lecture
                            </span>
                          )}
                        </div>
                  ...
                  {rest.map((actu) => (
                    <Link key={actu.id} to={`/actualites/${actu.id}`}
                      ...
                      <div className="relative h-48 overflow-hidden">
                        {getImageUrl(actu.image) ? (
                          <img
                            src={getImageUrl(actu.image)}
                            alt={actu.titre}
                            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                          />
                        ) : (
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-400">
                          <span className="flex items-center gap-1">
                            ...
                            {formatDate(actu.date_publication ?? actu.date)}
                          </span>
                          {actu.temps_lecture && (
                            <span className="flex items-center gap-1">
                              <Clock className="w-3 h-3" />{actu.temps_lecture}
                            </span>
                          )}
                        </div>
                      </div>
