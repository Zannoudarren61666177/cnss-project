...
import { getTravailleurs, getPrestations } from '../api';
...
export function EmployeurDashboard() {
  ...
  const [fetchedTravailleurs, setFetchedTravailleurs] = useState<Travailleur[]>([]);
  const [fetchedDeclarations, setFetchedDeclarations] = useState<Declaration[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setDataError(null);
      try {
        const [travailleursRes, prestationsRes] = await Promise.all([getTravailleurs(), getPrestations()]);

        const mappedTravailleurs: Travailleur[] = (travailleursRes || []).map((t: any) => ({
          id: t.id,
          nom: t.last_name ?? t.nom ?? t.lastName ?? '',
          prenom: t.first_name ?? t.prenom ?? t.firstName ?? '',
          matricule: t.cin ?? t.matricule ?? '',
          statut: 'Actif',
          dateAffiliation: t.created_at ? t.created_at.split('T')[0] : '',
          poste: t.position ?? t.poste ?? '',
          salaire: t.salaire ?? t.salary ?? undefined,
        }));

        const mappedDeclarations: Declaration[] = (prestationsRes || []).map((p: any, idx: number) => ({
          id: p.id ?? idx + 1,
          periode: p.date_debut ?? p.periode ?? `Prestation-${p.id ?? idx + 1}`,
          montant: p.montant ?? 0,
          statut: p.status === 'paid' ? 'Payée' : 'En attente',
          dateEcheance: p.date_fin ?? '',
          dateEmission: p.created_at ? p.created_at.split('T')[0] : '',
          secteurActivite: p.secteur ?? 'N/A',
          nombreTravailleurs: p.nombre_travailleurs ?? 0,
          masseSalariale: p.masse_salariale ?? 0,
          tauxCotisationSalariale: p.taux_salariale ?? 4.0,
          tauxCotisationPatronale: p.taux_patronale ?? 16.0,
          montantSalarial: p.montant_salarial ?? 0,
          montantPatronal: p.montant_patronal ?? 0,
          detailsCalcul: p.details_calcul ?? [],
        }));

        if (!mounted) return;
        setFetchedTravailleurs(mappedTravailleurs);
        setFetchedDeclarations(mappedDeclarations);
      } catch (err: unknown) {
        setDataError(err instanceof Error ? err.message : 'Erreur chargement données');
      } finally {
  ...
  const displayedTravailleurs = fetchedTravailleurs;
  const displayedDeclarations = fetchedDeclarations;
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
                ...
                <p className="text-3xl font-bold text-gray-900">{displayedTravailleurs.length}</p>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
                ...
                <p className="text-3xl font-bold text-gray-900">{displayedDeclarations.length}</p>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
                ...
                <p className="text-2xl font-bold text-gray-900">1,35M FCFA</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                {displayedDeclarations.slice(0, 3).map((declaration) => (
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {displayedTravailleurs.map((travailleur) => (
